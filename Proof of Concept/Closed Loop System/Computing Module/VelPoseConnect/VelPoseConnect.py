...
cmd7 = 'roslaunch waypoint_maker waypoint_loader.launch load_csv:=True multi_lane_csv:=/home/phuris/.autoware/data/path/moriyama_path.txt replanning_mode:=False realtime_tuning_mode:=False resample_mode:=True resample_interval:=1 replan_curve_mode:=False overwrite_vmax_mode:=False replan_endpoint_mode:=True velocity_max:=20 radius_thresh:=20 radius_min:=6 velocity_min:=4 accel_limit:=0.5 decel_limit:=0.3 velocity_offset:=4 braking_distance:=5 end_point_offset:=1;'

#run the node 'wf_simulator'
cmd8 = 'roslaunch waypoint_follower wf_simulator.launch initialize_source:=Rviz accel_rate:=1 position_error:=0 angle_error:=0 use_ctrl_cmd:=False;'

#run the node 'lane_rule'
cmd9 = 'rosrun lane_planner lane_rule;'

#run the node 'lane_stop'
cmd10 = 'rosrun lane_planner lane_stop;'

#run the node 'lane_select'
cmd11 = 'roslaunch lane_planner lane_select.launch search_closest_waypoint_minimum_dt:=5;'
...
time.sleep(5)
subprocess.call(['gnome-terminal','-x','/bin/bash','-i','-c',cmdnode14])
time.sleep(5)
subprocess.call(['gnome-terminal','-x','/bin/bash','-i','-c',cmdnode15])
time.sleep(5)
subprocess.call(['/bin/bash', '-i', '-c', cmdnode16])
